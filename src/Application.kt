...
fun Application.module(testing: Boolean = false) {
    ...
    routing {
        ...
        get("/library/book/{bookid}/reserve") {
            val bookid = call.parameters.get("bookid")
            call.respond(BookReserveResponse("You reserved the book $bookid", emptyList<HypermediaLink>()))
        ...
        get("/library/book/{bookid}") {
            ...
            val hypermedialinks = listOf<HypermediaLink>(
                HypermediaLink(
                    "http://localhost:8080/library/book/$bookid/checkout", "checkout", "GET"
                ),
                HypermediaLink(
                    "http://localhost:8080/library/book/$bookid/reserve", "reserve", "GET"
                )
            )
...
data class BookReserveResponse(val message: String, val links: List<HypermediaLink>)
